import { add, divide, contact } from "./units";
...
describe("concat", () => {
    it('should concat "Hello " with "world!"', () => {
        const msg = contact("Hello ", "world!");
        expect(msg).to.equal("Hello world!");
    });

    it("should throw an error if one of two strings is empty", () => {
        expect(() => {
            contact("", "Ooops");
        }).to.throw("Neither of two strings can be empty.");
    });
});
