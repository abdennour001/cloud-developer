...
(async () => {
    ...
    app.get("/cars", (req: Request, res: Response) => {
        const { make, type, model } = req.query;

        let cars_list = cars;

        cars_list = cars
            .filter(car => {
                return make !== undefined ? car.make === make : car;
            })
            .filter(car => {
                return type !== undefined ? car.type === type : car;
            })
            .filter(car => {
                return model !== undefined ? car.model === model : car;
            });
    ...
    app.post("/cars", (req: Request, res: Response) => {
        ...
        if (
            JSON.stringify({ id, make, type, model, cost }) ===
            JSON.stringify({})
        ) {
            return res.status(400).send("All parameters are required!");
        }

        const new_car: Car = { id, make, type, model, cost };
